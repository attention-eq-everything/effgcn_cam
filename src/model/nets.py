...
class EfficientGCN(nn.Module):
    def __init__(self, data_shape, block_args, fusion_stage, stem_channel, **kwargs):
        ...
        # output
        last_channel = (
            num_input * block_args[-1][0]
            if fusion_stage == len(block_args)
            else block_args[-1][0]
        )
    ...
    def forward(self, x):
        ...
        # Feed in the 3 type of inputs: joints, velocity and bone (I = 3)
        # Hence this permutation and reshaping is required
...
class EfficientGCN_Blocks(nn.Sequential):
    def __init__(
            self, init_channel, block_args, layer_type, kernel_size, input_channel=0, **kwargs
    ):
        ...
        if input_channel > 0:  # if the blocks in the input branches
            ...
            self.add_module('stem_scn', Spatial_Graph_Layer(
                input_channel, init_channel, max_graph_distance, **kwargs)
            )
            self.add_module('stem_tcn', Temporal_Basic_Layer(
                init_channel, temporal_window_size, **kwargs)
            )
        ...
        for i, [channel, stride, depth] in enumerate(block_args):
            self.add_module(f'block-{i}_scn', Spatial_Graph_Layer(
                last_channel, channel, max_graph_distance, **kwargs)
            )
            for j in range(depth):
                s = stride if j == 0 else 1
                self.add_module(f'block-{i}_tcn-{j}', temporal_layer(
                    channel, temporal_window_size, stride=s, **kwargs)
                )
            # (kapitthakkar's comment)
            # The attention layer is present at the end of each block in the graph
            # convolutional network, which basically gets the current layer's importance
            # features for each joint the video. This is propagated until the end,
            # where the final FC layer will have features coming in which are obtained
            # after all attention layers have been applied: that's the final feature vector
            # we want.
...
class EfficientGCN_Classifier(nn.Sequential):
    def __init__(self, curr_channel, num_class, drop_prob, **kwargs):
        ...
        # (kalpitthakkar's comment)
        # The input features for this layer are basically attention outputs from previous
        # layers and already contain the importance of each joint per frame in the video.
        # The weight parameters for this layer are the final learned parameter that perturb
        # these importance weights from the previous layers present in the input feature.
