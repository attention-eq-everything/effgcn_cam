...
class Visualizer():
    def __init__(self, args):
        ...
        self.action_names['ntu'] = [
            'drink water 1', 'eat meal/snack 2', 'brushing teeth 3', 'brushing hair 4',
            'drop 5', 'pickup 6', 'throw 7', 'sitting down 8', 'standing up 9', 'clapping 10',
            ...
            'put on a hat/cap 20', 'take off a hat/cap 21', 'cheer up 22',
            'hand waving 23', 'kicking something 24', 'put/take out sth 25', 'hopping 26',
            'jump up 27', 'make a phone call 28', 'playing with a phone 29', 'typing on a keyboard 30',
            ...
            'put the palms together 39', 'cross hands in front 40', 'sneeze/cough 41', 'staggering 42',
            'falling 43', 'touch head 44', 'touch chest 45', 'touch back 46', 'touch neck 47',
            'nausea or vomiting condition 48', 'use a fan 49', 'punching 50', 'kicking other person 51',
            'pushing other person 52', 'pat on back of other person 53', 'point finger at the other person 54',
            'hugging other person 55', 'giving sth to other person 56', 'touch other person pocket 57',
            'handshaking 58', 'walking towards each other 59', 'walking apart from each other 60'
        ]
    ...
    def read_data(self):
        ...
        # Okay, so here's where all the final pieces are put together to
        # plot the joint importances on the final skeletons, the video's
        # predicted / ground truth action class and the probability of the
        # predicted class.
        # To understand the calculation of self.result, I'd recommend to
        # give https://pytorch.org/docs/stable/generated/torch.einsum.html
        # a read. It's not possible to explain it in comments.
        # In short though, the input operands have dimensions (K, C): weights
        # and (C, T, V, M): features, which need to be multiplied element-wise
        # along the C dimensions and the output has dimensions (K, T, V, M)
        # which means there's no dimension that's ommitted in the output. We
        # are multiplying the final layer's weights to the input features.
        # In fact, we are just mimicking the final FC layer.
    ...
    def show_confusion_matrix(self):
        ...
        sns.heatmap(cm, cmap=plt.cm.Blues, annot=True, annot_kws={'fontsize':font_size-2}, cbar=False,
                    square=True, linewidths=0.1, linecolor='black', xticklabels=show_name_x,
                    yticklabels=show_name_y
        )
    ...
    def show_NTU_skeleton(self):
        ...
        # (kalpitthakkar's comment)
        # The result matrix is set to give the importances for the predicted class,
        # which will have the maximum value in result for each input.
        # Each value along the V dimension for that class (i.e the joint importances)
        # are proportional to it's contribution for the prediction of the class.
        # The result matrix is normalized as we're using these values to plot colors
        # (keeping them between 0 - 1 will assign the intensities accordingly).
        ...
        for t in frames:
            ...
            for m in range(M):
                ...
                for v in range(V):
                    # (kalpitthakkar's comment)
                    # Don't really know the purpose of dividing by 4, but I'd
                    # guess it is to keep the output colors a little smooth.
                    # Otherwise, we might get jittery colors as the numbers can
                    # keep changing by little amounts across adjacent frames.
                    r = result[t//4,v,m]
                    g = 0
                    b = 1 - r
                    c.append([r,g,b])
